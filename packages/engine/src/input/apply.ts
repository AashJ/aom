...
export function applyInput(
  ...
  if (input.dragging) {
    ...
    if (screenToGround(camera, ndcX, ndcY, scratchA)) {
      if (!input.hasDragAnchor) {
        ...
        camera.target[0] = camera.target[0]! + input.dragAnchorX - scratchA[0]!;
        camera.target[2] = camera.target[2]! + input.dragAnchorZ - scratchA[2]!;
  ...
  if (input.wheelDelta !== 0) {
    ...
    // Matrices must reflect the new distance before intersecting the cursor ray again.
    updateMatrices(camera, clientWidth / clientHeight);

    if (before && screenToGround(camera, ndcX, ndcY, scratchB)) {
      camera.target[0] = camera.target[0]! + scratchA[0]! - scratchB[0]!;
      camera.target[2] = camera.target[2]! + scratchA[2]! - scratchB[2]!;
      // A zero pan reuses the camera bounds clamp without moving in view axes.
      pan(camera, 0, 0);
    }
