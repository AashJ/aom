...
import { definition } from "./atalanta";

const reference = GATE_C_UNIT_REFERENCES.find((entry) => entry.key === "greek-atalanta");
if (reference === undefined) throw new Error("Atalanta requires a Gate C reference.");
...
describe("Atalanta unit pack", () => {
  ...
  test("pins the original twenty-eight-tick melee cycle", () => {
    expect(definition.attack.cooldownTicks).toBe(28);
  });

  test("applies the seven-times myth counter after target armor", () => {
    expect(resolveMeleeDamage(definition.attack, target(UNIT_CLASS_HUMAN, [0.25, 0, 0]))).toBe(6);
    expect(resolveMeleeDamage(definition.attack, target(UNIT_CLASS_MYTH, [0.25, 0, 0]))).toBe(42);
  });
});
