...
### C3 — serial Minotaur myth and charged-action foundation
...
5. **Candidate evidence.** The local Trial proto pins the Classic gameplay row and target set; the original root animation pins a 47% impact in the two-second Gore clip. The candidate reference and test spawn may be used before final integration, but the roster remains `ready` until a review commit closes the evidence and implementation together.

The original Gore also launches its victim. Its trajectory, airtime, landing collision, and commandability are engine behavior rather than values in the available proto/root-animation sources. C3 must not invent those constants: the Minotaur candidate can validate charge selection, cancellation, impact damage, recharge, and original presentation, but final `implemented` status and myth fan-out require a separately pinned deterministic target-reaction contract. A presentation-only knockback is forbidden because displacement and disable time affect gameplay.
