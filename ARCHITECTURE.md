...
| Accuracy and dodging   | B1 captures a fixed target point at release and does not home afterward, so a moving target is hit only if its body overlaps that point at impact time. Classic `Accuracy`, `AimBonus`, `MaxSpread`, `SpreadFactor`, `TrackRating`, and related path-hit behavior remain B2: raw proto values are evidence, not permission to invent the engine formula.  |
...
2. **B2 — Classic accuracy and collision fidelity.** Pin the ordinary ranged formula for aim/spread/tracking from fidelity evidence; implement accuracy misses plus path collision against projectile-collidable bodies, deterministic collision ordering, unintentional damage, and the exact friendly/neutral obstruction rules without an all-units quadratic scan.
