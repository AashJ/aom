...
| Accuracy and dodging   | B1 captures a fixed target point at release and does not home afterward. B2 targets Extended Edition 2.8's inherited legacy projectile semantics, not the 2002 vanilla balance table. The Trial proto/executable may establish field meaning and engine shape, but `Accuracy`, `AimBonus`, `MaxSpread`, `SpreadFactor`, `TrackRating`, and ownership behavior stay gated wherever EE evidence is missing. |
...
| Parallel workcell gate | A ranged lane stays `blocked` until the serial vertical slice proves every behavior it needs, its producer/slot and final-ruleset reference are frozen, its projectile type/media exists, and focused validation rejects omitted projectile fields. Definition presence alone never opens a lane.                                                         |
...
2. **B2 — Extended Edition 2.8 accuracy and collision fidelity.** First mirror the authored `CollidesWithProjectiles` body policy, reuse the deterministic unit grid, and prove swept intended-target contact. Then pin the ordinary EE formula for aim/spread/tracking from fidelity evidence and implement accuracy misses, deterministic unintended path hits, unintentional damage, and the exact friendly/neutral obstruction rules. The milestone is not complete—and no ranged lane opens—until both cuts are proven without an all-units quadratic scan.
3. **B3 — projectile presentation.** Add stable projectile media definitions and models, render snapshot projectiles with interpolation and launch/arc/impact presentation, bind release timing to the authored attack action, and cover device recreation/model-registry validation.
...
6. **B6 — ranged roster release.** Audit all five ordinary Greek/Egyptian projectile candidates against the proven contract, freeze producer/slot/projectile assignments and references, mark only eligible lanes `ready`, and generate isolated lane briefs. A unit with an unproven mechanic remains blocked with that exact mechanic named.

### B2 fidelity evidence boundary

- **Ruleset authority:** Extended Edition 2.8 / The Titans data and reproducible EE behavior. A future local `protox` source must be hashed and verified before it can freeze ranged unit references.
- **Legacy corroboration only:** the local Trial proto (`464520f1ea00b36e1872bf5a59831408c819c205e56f055c7b2e8bdf53719da2`) and extracted Trial executable (`5975176380f29104c66e49fa7dc73d2a24221612190de630258f8523f7825366`) establish the named accuracy parameters and authored `CollidesWithProjectiles` flag. A static executable audit also pins the legacy hit-score shape (`base accuracy * 100 - distance * reduction + aim bonus * prior shots`), but it does not prove EE values, aim-reset rules, spread sampling, tracking/lead behavior, or relationship filtering.
- **Implemented first cut:** projectile solidity is explicit per unit type; resource nodes are not made solid merely because they have a body radius; released shots use continuous swept contact against their intended solid target; the existing deterministic unit grid now has one shared rebuild owner for the later unintended-body query.
- **Still blocked:** EE aim accumulation, spread sampling, track/lead behavior, unintended-damage selection and multiplier, ally pass-through, and neutral obstruction. No agent may infer these from field names or a community formula.
...
- Every `ready` ordinary-projectile lane needs only its owned sim definition, media definition, assets, and focused test; it cannot edit shared projectile code, schemas, roster references, or generated catalogs.
