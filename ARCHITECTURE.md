...
- **Pinned ordinary values:** Toxotes, Peltast, Slinger, and Chariot Archer author `Accuracy 0.8`, `AccuracyReductionFactor 1.5`, `AimBonus 15`, `SpreadFactor 0.25`, `MaxSpread 5`, `TrackRating 5`, and `UnintentionalDamageMultiplier 0.3`. The Trial `Crossbowman` row uses `Accuracy 0.6` with the other six values unchanged; it may not freeze Gastraphetes until that identity mapping is separately evidenced.
